...
export function buildCriticPrompt(
  ...
  const commentsFormatted = comments
    .map((c, index) => {
      let header = `Comment #${index + 1} (${c.type.toUpperCase()})`;
      if (c.file) header += ` File: ${c.file}`;
      if (c.line !== undefined) header += ` Line: ${c.line}`;
      if (c.phase) header += ` (Phase: ${c.phase})`;
      return `--- ${header} ---\n${c.comment}\n--- End Comment #${index + 1} ---`;
    })
    .join('\n\n');
  ...
  if (personaGuidelines) {
    personaInstruction = `\n--- REVIEW PERSONA GUIDELINES ---\nFor the review comments being evaluated, the automated review phase reviewer used the following persona focus guidelines:\n${personaGuidelines}\n\nIMPORTANT: Take these focus guidelines into account.\n-----------------------------------\n`;
  }

  return `You are a highly analytical, objective software engineering Critic and Lead Code Reviewer.
Your task is to critically review and evaluate a set of proposed code review comments that were generated by automated phase-based code reviewers for a Pull Request.

You have access to the working directory of the repository. Use your file inspection tools to inspect code files in the codebase whenever necessary.
...
export class PRReviewerService {
  ...
  async critiqueComments(
    ...
    try {
      ...
      const effectiveRepoPath = options.repoPath
        ? this.getEffectiveRepoPath(options.repoPath)
        : undefined;
      const sessionOpts = effectiveRepoPath
        ? { workingDirectory: effectiveRepoPath }
        : {};

      const model = options.modelOverride || settings.copilotModel;
      const clientAndSession = await this.copilotService.createClientAndSession(
        settings.copilotToken,
        model,
        sessionOpts,
      );
      ...
      const { client, session } = clientAndSession;
      session.isPrReviewer = true;
      session.label = 'PR Reviewer Phase: Critic';
      ...
      const handledOriginalIndices = new Set<number>();
      const critiquedComments: ReviewComment[] = [];
      ...
      for (const rawLine of lines) {
        ...
        try {
          ...
          if (
            obj.action === 'approve' &&
            typeof obj.commentIndex === 'number'
          ) {
            const idx = obj.commentIndex - 1;
            if (
              idx >= 0 &&
              idx < comments.length &&
            ...
            if (
              ...
              critiquedComments.push({
                ...comments[idx],
                status: 'rejected',
                reason: obj.reason || 'Rejected by Critic',
              });
            }
          } else if (
            obj.action === 'merge' &&
            Array.isArray(obj.commentIndices) &&
            typeof obj.comment === 'string'
          ) {
            const indices = obj.commentIndices
              .map((n: number) => n - 1)
              .filter((i: number) => i >= 0 && i < comments.length);

            if (indices.length > 0) {
              const unhandled = indices.filter(
                (i: number) => !handledOriginalIndices.has(i),
              );
      ...
      // Fallback for any unhandled comments: mark as approved
      comments.forEach((c, idx) => {
        if (!handledOriginalIndices.has(idx)) {
          critiquedComments.push({
            ...c,
            status: 'approved',
          });
      ...
      const matchedModel = availableModels.find(
        (m) =>
          m.id.toLowerCase() === (usage.model || model || '').toLowerCase() ||
          m.name.toLowerCase() === (usage.model || model || '').toLowerCase(),
      );
      const modelDisplayName = matchedModel
        ? matchedModel.name
        : usage.model || model || 'Critic';
      ...
      try {
        ...
        console.error('Error destroying session in critiqueComments:', e);
      }
      try {
        await client.stop();
      } catch (e) {
        console.error('Error stopping client in critiqueComments:', e);
      }

      return {
        result: critiquedComments,
        usage,
      };
    } finally {
      if (blockerId !== null && powerSaveBlocker.isStarted(blockerId)) {
